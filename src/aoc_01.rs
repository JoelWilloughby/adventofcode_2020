...
pub fn read_it(filename: &str) -> Result<Vec<usize>> {
...
pub fn vec_to_bool_array(nums: &Vec<usize>) -> [bool; 2021] {
...
pub fn search_for_two(nums: &Vec<usize>, num_set: &[bool; 2021]) {
    for num in nums.iter() {
...
pub fn search_for_three(nums: &Vec<usize>, num_set: &[bool; 2021]) {
    for num in nums.iter() {
...
mod tests {
    ...
    #[test]
    fn test_it() {
        let nums = read_it("res/01/input.txt").unwrap();
